...
//     password: { type: String, required: function () { return !this.googleId; } }, // Required only if not a Google user
...
// // Middleware to delete related contacts and profile before a user is deleted
...
const UserSchema = new mongoose.Schema(
  {
    ...
    password: {
      type: String,
      required: function () {
        return !this.googleId; // Ensures password is required only if not a Google user
      },
    },
    role: { type: String, enum: ["user", "admin"], default: "user" },
    isVerified: { type: Boolean, default: false },
